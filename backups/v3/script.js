...
themeToggleBtn.addEventListener('click', () => {
    ...
    if (isDarkMode) {
        ...
        themeToggleBtn.title = "עבור למצב בהיר";
    } else {
        map.removeLayer(darkLayer);
        map.addLayer(lightLayer);
        themeToggleBtn.textContent = '🌙';
        themeToggleBtn.title = "עבור למצב כהה";
    }
});
...
flows.forEach(flow => {
    try {
        ...
        polyline.bindTooltip(`
            <div class="popup-title">${flow.from} &larr; ${flow.to}</div>
            <div class="popup-stat">כמות: ${flow.count}</div>
        `, {
            sticky: true, // Follow mouse
            direction: 'top',
            className: 'custom-tooltip' // We'll reuse popup styles or add new ones
        });
        flow.layers.push(polyline);

        // 2. Animated Overlay Line (White Dash)
        const animPolyline = L.polyline(latlngs, {
            color: 'white',
            weight: Math.max(2, getWeight(flow.count) / 3), // Thinner than base
            opacity: 0.6,
            dashArray: '10, 80', // Short dash, long gap
            className: 'flow-anim' // CSS Animation
        }).addTo(map);
